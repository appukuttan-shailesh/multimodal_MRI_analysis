...
from nilearn.glm import threshold_stats_img, compute_contrast
...
json_filename = data_sub_dir + 'sub-{0:s}_ses-02_{1:s}_hemi-R_space-fsaverage_bold.json'.format(sub, task)
data_filename = data_sub_dir + 'sub-{0:s}_ses-02_{1:s}_hemi-R_space-fsaverage_bold.func.gii'.format(sub, task)
...
data_img = [x.data for x in data_img.darrays]
data_hemiR = np.vstack(data_img)
n_scans = data_hemiR.shape[0] # the acquisition comprises 128 scans
...
#
contrast_glm = compute_contrast(labels, estimates, contrast_cond,
                                contrast_type='t')

# z-score map
z_map = contrast_glm.z_score()
...
plotting.plot_surf_stat_map(
     curv_mesh, z_map, hemi='right',
     title=contrast, colorbar=True,
     threshold=2.5, bg_map=sulc_surf,
     output_file=out_sub_dir+'{}_contrast_surf_{}.png'.format(task, contrast))
